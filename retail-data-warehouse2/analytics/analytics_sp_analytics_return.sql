create table if not exists analytics.return2(date date, product_code varchar(255),store smallint,order_number varchar(255),country varchar(100),
...
create or replace procedure analytics.sp_analytics_return()
language plpgsql as $$
begin

truncate table analytics.return2; 
insert into analytics.return2
...
where is_primary = 'True' and rm.quantity is not NULL);  
